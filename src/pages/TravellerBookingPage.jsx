...
import { FaArrowRight, FaBusAlt, FaCalendarAlt, FaChevronDown, FaStar } from 'react-icons/fa'
...
function SelectField({ label, value, options = [value], className = '' }) {
  return (
...
function TextField({ label, value, placeholder, className = '', muted = false, calendar = false }) {
  return (
    <FloatingField label={label} className={className}>
      <input
        defaultValue={value}
...
export default function TravellerBookingPage() {
            ...
            {bookingSteps.map((step, index) => (
              <React.Fragment key={step.number}>
                <div className="flex h-16 shrink-0 flex-col items-center gap-2">
                  <span
                    className={`relative flex h-10 w-10 items-center justify-center rounded-full text-[16px] font-bold leading-6 text-white ${
                      step.active
        ...
        <section className="bg-[#f6f7f7] px-4 pb-16">
          <div className="mx-auto grid w-full max-w-[1216px] gap-8 lg:grid-cols-[minmax(0,800px)_389px]">
            <div className="rounded-[24px] border-[0.4px] border-[#4D4D4D] bg-white p-5 shadow-[0_20px_25px_-5px_rgba(0,0,0,0.05),0_8px_10px_-6px_rgba(0,0,0,0.05)] sm:p-8">
              <h2 className="mb-8 flex items-center gap-2 text-[20px] font-bold leading-8 text-[#444444] sm:text-[24px]">
                <FaBusAlt className="text-[18px] text-[#748E36]" />
              ...
              <div className="grid gap-6">
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <SelectField label="Vehicle Type" value="AC Traveller / Minivan" className="md:mt-2" />
                  <SelectField label="Vehicle Name" value="12 Seater (2+1) Force Traveller" className="md:mt-2" />
                </div>
                ...
                <div className="space-y-3">
                  <div className="relative">
                    <input
                      ...
                      className={`${fieldBase} placeholder:text-[#4D4D4D]/60`}
                    />
                    <button
                      type="button"
                      className="absolute right-4 top-full mt-1 text-[10px] font-medium leading-3 text-red-500"
                    >
                      Remove
                    </button>
                  </div>
            ...
            <aside className="space-y-4">
              <div className="relative overflow-hidden rounded-[20px]">
                <img
                  src={travellerPreviewImage}
                  alt="12 Seater Force Traveller"
          ...
          <div className="mx-auto mt-8 flex w-full max-w-[1218px] flex-col items-center justify-between gap-5 sm:flex-row">
            ...
            <button
              type="button"
              onClick={() =>
                navigate('/rent-traveller/routes/view-buses/detail/book-now/passenger-details', {
